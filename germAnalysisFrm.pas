...
type
    ...
    procedure chkInputKeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  //id:=getid;
  if bNewPatient then
   hpr:=zHelper.create(js,'ypbase',self,bHumanInput,bNoScanInput,bNewPatient,0)
  else
   hpr:=zHelper.create(js,'ypbase',self,bHumanInput,bNoScanInput,bNewPatient,dmym.rsbase['useid']);
...
begin
   if (bhumanInput=true) or (bNewPatient=false) then
   begin
      if cb_germtype.Text='' then
      begin
        showMessage('请选择菌属');
  ...
  //程序重复一次。解决细菌分析要点击两次才正确的BUG的零时方案
  if (bhumanInput=true) or (bNewPatient=false) then
   begin
      if cb_germtype.Text='' then
      begin
...
procedure TgermAnalysisForm.chkInputClick(Sender: TObject);
begin
   bHumanInput:=chkInput.Checked;
   btnstartscan.Enabled:=not(chkinput.Checked);
   if bHumanInput then
   begin
      btnAnalysis.Enabled:=true;
      btnAnalysis.SetFocus;
     // btnYmAnalysis.Enabled:=true;
   end;
end;
...
procedure TgermAnalysisForm.CheckBox1Click(Sender: TObject);
begin
  bNoScanInput:=checkbox1.Checked;
...
procedure TgermAnalysisForm.chkInputKeyPress(Sender: TObject;
  var Key: Char);
begin
    if key=#13 then
    begin
        if not chkinput.Checked then
            chkinput.Checked:=true
        else
...
procedure TgermAnalysisForm.CheckBox1KeyPress(Sender: TObject;
  var Key: Char);
begin
    if key=#13 then
    begin
